      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 container">
        {services.map((service, index) => (
          <Link href="/contact" key={index} passHref> 
            <div
              className="h-full border-[0.799px] border-[#122F26] bg-[rgba(0,0,0,0.27)] p-6 flex flex-col text-left group transition-all duration-300 ease-in-out cursor-pointer hover:border-[#122F26] hover:bg-greenhover hover:backdrop-blur-[18px]"
            >
              <Image src={service.logo} alt={`${service.title} Logo`} className="h-16 mb-4 text-left" />
              <h2 className="text-lg font-semibold text-white mb-4 text-left group-hover:text-black">{service.title}</h2>
              <p className="text-base font-bold text-gray-300 text-left group-hover:text-black">{service.description}</p>
            </div>
          </Link>
        ))}
      </div>
