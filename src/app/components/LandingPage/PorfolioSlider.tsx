...
function PortfolioSlider({
  ...
  return (
    <Link
      href={
        slides[currentSlideIndex]?.link ?? slides[currentSlideIndex - 1]?.link
      }
      className=" text-white relative w-full  px-6 md:px-14 py-10 flex flex-col items-center container"
    >
      <button
        type="button"
        className="focus:outline-none absolute swiper_prev z-10 left-4 top-[50%] -translate-y-1/2"
      >
        <Image src={leftArrow} alt="Previous Slide" />
      </button>
      <button className="focus:outline-none absolute swiper_next z-10 right-4 top-[50%] -translate-y-1/2">
        ...
        {(slides.length > 0 && slides.length <= 3
          ? [
...

              {
                ...
                link: slides[slides.length - 1]?.link ?? "",
              },
            ]
          : slides
        ).map((slide, index) => (
